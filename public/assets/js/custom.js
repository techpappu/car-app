...
/////////////////////////////////////////////////////////////////
  ...
  if ($('#slider-price').length > 0) {
    ...
    sliderPrice.noUiSlider.on('update', function( values, handle ) {
      snapValuesPrice[handle].innerHTML = values[handle];
    });
  }
  ...
  if ($('#slider-year').length > 0) {
    ...
    var snapValuesYear = [
      document.getElementById('year-slider-snap-value-lower'),
      document.getElementById('year-slider-snap-value-upper')
    ];

    sliderYear.noUiSlider.on('update', function( values, handle ) {
      snapValuesYear[handle].innerHTML = values[handle];
    });
  }
  ...
  if ($('#slider-mileage').length > 0) {
    ...
    var snapValuesMileage = [
      document.getElementById('mileage-slider-snap-value-lower'),
      document.getElementById('mileage-slider-snap-value-upper')
    ];

    sliderMileage.noUiSlider.on('update', function( values, handle ) {
      snapValuesMileage[handle].innerHTML = values[handle];
    });
  }

  if ($('#slider-displacement').length > 0) {
    ...
    var snapValuesDisplacement = [
      document.getElementById('displacement-slider-snap-value-lower'),
      document.getElementById('displacement-slider-snap-value-upper')
    ];

    sliderDisplacement.noUiSlider.on('update', function( values, handle ) {
      snapValuesDisplacement[handle].innerHTML = values[handle];
    });
  }

  if ($('#slider-seating').length > 0) {
    var slider = document.getElementById('slider-seating');
    ...
    var snapValueSeating = [
      document.getElementById('seating-slider-snap-value-lower'),
      document.getElementById('seating-slider-snap-value-upper')
    ];

    slider.noUiSlider.on('update', function( values, handle ) {
      snapValueSeating[handle].innerHTML = values[handle];
    });
  }
  ...
  $.ajax({
      ...
      success: function(response) {
         ...
         // $(".cartItemCount").text(parseInt(count) + 1);
           ...
           $(".cartItemCount").text(parseInt(count[0].innerHTML)+1);

          if (response.hasError == false) {
            notification("<div id='noti' class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><p>Successfully added to cart.</p></div>");
          } else {
...
function notification(data){
  ...
  setTimeout(function() { $('#noti').alert('close'); }, 2000);
}